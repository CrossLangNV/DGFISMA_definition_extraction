        ...
        if os.path.exists(MODEL_PATH):
            if os.path.exists(os.path.join(MODEL_DIR, 'config.json')):
                ...
                if config['model_type'] == "distilbert":
                    ...
                    raise ValueError(
                        f"config.json located at {MODEL_DIR} is not a valid configuration file. Model type not supported or not available")
    ...
    def get_definitions_fasttext(self, model_path, pos_label='__label__definition', neg_label='__label__no_definition'):
        ...
        try:
            ...
            raise AssertionError(
                f'fastText model labels {model.labels} should be same as "pos_label: {pos_label}" and "neg_label: {neg_label}".')
